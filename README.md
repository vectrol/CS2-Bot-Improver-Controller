...
- **外观自定义** — 6 种主题色预设 + 自定义色值、紧凑模式
- **Steam 启动选项** — 自定义启动参数（自动合并 -insecure）
- **数据备份** — 导出/导入设置、收藏与自定义命令（JSON）
- **包完整性** — 内置整合包 SHA-256 校验，损坏包拒绝安装
- **导播观战模式** — 一键进入观战席（自动加入 spectator + 自动导播开关），GSI 实时遥测驱动表层覆盖 HUD（Lexogrine HUD 风格）：CT/T 比分与存活、回合计时、爆炸红色脉冲、玩家双列列表（击杀/死亡/金钱/主武器/HP 条/回合计分），覆盖层高度自定义（位置预设/拖动/透明度/字号/点击穿透/显示项/自动导播）
- **首页快捷操作** — 飞碟模式/重开对局/重随皮肤等常用命令一键复制
- **版本检查** — 启动自动检查上游插件包与 CBIC 控制器 GitHub Release，发现新版首页横幅提示
- **卸载** — 一键移除全部插件文件并恢复在线模式
- 简体中文 / 繁體中文 / English 三语，暗色主题 + SVG 图标（lucide-react），流畅 CSS 动画
